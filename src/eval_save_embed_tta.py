...
def build_transform(is_train, config):
    ...
    t = []
    t.append(transforms.RandomResizedCrop(config.DATA.IMG_SIZE, scale=(0.3, 1.0)))
    t.append(transforms.RandomHorizontalFlip(p=0.5))
    t.append(transforms.RandomApply([transforms.ColorJitter(brightness=0.4,contrast=0,saturation=0)],p=0.5))
    t.append(transforms.RandomApply([transforms.ColorJitter(brightness=0,contrast=0.4,saturation=0)],p=0.5))
    t.append(transforms.RandomApply([transforms.ColorJitter(brightness=0,contrast=0,saturation=0.4)],p=0.5))
...
with open("df_query"+opt.resume.split("_")[-1].split(".")[0]+"_swinL_ddp_384-48-"+str(seed)+".pkl", "wb") as f:
    # 将DataFrame对象写入文件中
    pickle.dump(df_query, f)

with open("df_gallery"+opt.resume.split("_")[-1].split(".")[0]+"_swinL_ddp_384-48-"+str(seed)+".pkl", "wb") as f:
    # 将DataFrame对象写入文件中
